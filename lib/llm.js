...
9. Prefer comprehensive function-level block comments over cluttered inline comments. Quality over quantity.`;
    ...
    // Project fingerprint + file skeleton give the model broad architectural awareness across chunks. [ds]
 ...
 */
/** [ds]
 * Regex-only skeleton extractor: avoids any AST library to keep zero runtime dependencies.
 * Scans every line for require/import, function/class declarations, and module.exports.
 * Truncates each captured line to 80 chars so the skeleton stays well under the token budget.
*/
function buildFileSkeleton(code) {
    ...
    for (const line of lines) {
        const t = line.trim();
        // Capture CommonJS require() and ES6 import statements [ds]
        if (/^(const|let|var)\s+.+\s*=\s*require\s*\(/.test(t) || /^import\s+/.test(t)) {
            imports.push(t.slice(0, 80));
        // Capture module.exports and named ES6 export statements [ds]
        } else if (/^module\.exports/.test(t) || /^export\s+(default\s+)?/.test(t)) {
            exports.push(t.slice(0, 80));
        // Capture top-level function and class declarations (async-aware) [ds]
        } else if (/^(async\s+)?function\s+\w+/.test(t) || /^class\s+\w+/.test(t)) {
 ...
 * A project fingerprint and file skeleton are prepended to every chunk prompt so the model
 * has architectural awareness even when processing the middle or end of a large file.
*/
async function getComments(code, language, config, mode = 'default', projectContext = '') {
    ...
    // Build a compact file skeleton (imports, declarations, exports) for cross-chunk architectural grounding [ds]
    const fileSkeleton = buildFileSkeleton(code);
    // Combine project fingerprint and file skeleton into one reusable context block passed to every chunk [ds]
