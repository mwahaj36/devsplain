/** Asynchronously fetches a URL with retry mechanism, handling rate limiting and server errors */
async function fetchWithRetry(url, options, maxRetries = 3, initialDelay = 1000) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        // Catch and handle any exceptions during the fetch
        } catch (err) {
            clearTimeout(timeoutId);
        ...
        // If this isn't the last attempt, calculate the backoff delay
...
/** Analyzes the given code and returns an array of objects containing line numbers and comments */
async function getComments(code, language, config, mode = 'default') {
    ...
    let prompt = "";
    // Determine the mode of operation for comment generation
    if (mode === 'clean') {
        prompt = `
...
${numberedCode}
    ...
    // Check the provider for the API request
    if (config.provider === 'gemini') {
        ...
        // Extract the text response from the API data
        if (data.error) {
        ...
        // Alternative API request for different providers
    ...
    // Validate the parsed JSON array
