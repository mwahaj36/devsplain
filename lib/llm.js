...
// Tunables for the text chunker: chunk size, sliding-window overlap (preserves [ds]
// context at boundaries), and the size above which a file is chunked at all. [ds]
// ─── Chunking Constants ───────────────────────────────────────────────────────
...
/** [ds]
 * Reset the adaptive concurrency controller between runs.
 * @param {number} initialLimit - Starting concurrency level (default 2).
*/
/**
...
async function fetchFromProvider(prompt, config) {
    ...
    // Default branch: OpenAI-compatible endpoints (openai, groq, openrouter, local proxies, etc.) all share the chat-completions schema. [ds]
        ...
        // Groq enforces a lower max_tokens ceiling; exceeding it yields a hard 400, so clamp to 1000 for groq only. [ds]
