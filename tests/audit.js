...
const tests = [
  ...
  // --- C-like languages (C, C++, C#, Java, Go, Rust, Swift, Kotlin, Dart) ---
  {
  ...
  // --- Shell / Bash ---
  {
    name: 'Bash Bug Check: Quotes NOT resetting at line ends (multiline strings)',
    ext: '.sh',
    code: 'echo "hello\n# not a comment\nworld"',
    comments: [],
    mode: 'clean',
...
for (const t of tests) {
  try {
    ...
    console.log(`  Expected:\n${t.expected.replace(/^/gm, '    ')}`);
    console.log(`  Got:\n${err.actual ? err.actual.replace(/^/gm, '    ') : err.message}`);
    failed++;
  }
}
